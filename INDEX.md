...
- [An agent's Markdown is rendered by the client, so math survives in one surface and not another](entries/tools/reply-rendering-is-client-specific.md) — A terminal chat view shows `$x^2$` literally while a phone client typesets it, and the producing side never sees the difference. `agents`, `markdown`, `latex`
