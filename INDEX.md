...
- [Markdown clients routinely disable single-`$` math, so inline formulas fall through as literal text](entries/tools/single-dollar-math-often-disabled.md) — KaTeX is loaded and `$$…$$` works while `$x$` prints verbatim; the display/inline split is the diagnostic. `markdown`, `latex`, `katex`
